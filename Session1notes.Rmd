...
Here's a redux done after doing some more pattern logic: just stick with the `gsub()` strategy `age` conforms with `"lower to upper"`. Then we can define a new column called `age_interval`, bearing in mind the irregularity of labels. I decided NOT to throw out NAs yet. Some of them should be 0s, others are actually NAs, and we're not ready to detect which are which.
```{r}
MIG2 <-
  MIG %>% 
  select(-contains("junk")) %>% 
  ...
  mutate(age_interval = as.numeric(upper) - as.integer(age),
         ...
         immigrations = ifelse(is.na(immigrations),0,immigrations)) %>% 
  group_by(destination_country, year, cat) %>% 
  mutate(
    immigrations = as.numeric(immigrations),
    keep = sum(immigrations) > 0) %>% 
  ungroup() %>% 
  filter(keep) %>% 
  group_by(destination_country, year, cat, age) %>% 
  mutate(keep = age_interval == min(age_interval)) %>% 
  ungroup() %>% 
  group_by(destination_country, year, cat, age_interval) %>%
  mutate(keep = case_when(is.infinite(age_interval) ~ age == min(age),
                          age %in% c("Total","Unknown") ~ TRUE,
                          TRUE ~ keep)) %>% 
  ungroup() %>% 
  filter(keep) %>% 
  select(destination_country, year, cat, age, age_interval, immigrations)

MIG2 %>% filter(destination_country == "Austria", cat == "Total", year == 2010) %>% View()
```
...
subquestion: does each subgroup actually have a total? Here's a quick way to check
```{r}
MIG2$age %>% unique()
```
So, it's hard to say so far. How many distinct subgroups are there, and does each contain a total?

```{r}
MIG2 %>% select(destination_country, year, cat) %>% distinct() %>% nrow()
sum(MIG2$age == "Total")
```
